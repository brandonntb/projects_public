...
from keras.layers import Dropout, Flatten, Rescaling
...
# create_batch_fig(9, 3, 3, aug_trainX, aug_trainY)
...
early_callback = EarlyStopping(monitor="val_loss", min_delta=0.0001, mode="min", restore_best_weights=True, patience=5)
reduce_lrn = ReduceLROnPlateau(monitor="val_loss", patience=1, verbose=1, factor=0.3, min_lr=0.0001)
...
print('\n-+-+-+[Module Evaluation Finished]-+-+-+')

# ----------------------------------------------------

print('\nStarting Analysis of Convolutional Layers...')
# currently random ones from the internet, change to some from test set before submission
# img_path = "../Dataset - OCT/OCT2017/test/DRUSEN/DRUSEN-1219727-1.jpeg"padd
